...
class ResourcePicker {
  ...
  ResourcePicker(String clientId, String clientSecret, String enterpriseId, String redirectURL, {this.resourcePickerTimeoutDuration = const Duration(minutes: 5), this.testing = 0}) {
    this._clientId = clientId;
    this._clientSecret = clientSecret;
    this._enterpriseId = enterpriseId;
    this._redirectURL = redirectURL;
    _resourcePickerURL ="$_pcmUrl/auth?client_id=$_clientId&redirect_uri=$_redirectURL&response_type=code&scope=$_scope&state=state";
  }

  Future<String> askForAuthorization() async {
    try {
      var authClient = await _launchResourcePicker(_resourcePickerURL).timeout(resourcePickerTimeoutDuration);
